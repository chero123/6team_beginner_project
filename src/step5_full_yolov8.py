...
YOLO_BASE = os.path.join(BASE_PROJECT, "yolo_full")
...
def build_full_yaml():
    yaml_data = {
        ...
        "val": os.path.join(YOLO_BASE, "images"),  # full train이므로 val은 train과 동일
        "names": cat_names,
        "nc": num_classes
    }
...
def prepare_full_dataset():
    ...
    for img_name in os.listdir(TRAIN_IMG_DIR):
        ...
        dst_img = os.path.join(YOLO_BASE, "images", img_name)
        shutil.copy2(src, dst_img)

        dst_label = os.path.join(YOLO_BASE, "labels", img_name.replace(".png", ".txt"))
        write_label(img_name, dst_label)


# FULL TRAIN 실행
def run_full_train_yolo(epochs=150):
    yaml_path = build_full_yaml()
    prepare_full_dataset()

    model = YOLO(os.path.join(BASE_PROJECT, "yolov8m.pt"))
    ...
    print("\n🎉 YOLOv8m Full Training 완료!")
    print("📁 Best weight:", results.save_dir)


if __name__ == "__main__":
    run_full_train_yolo(epochs=150)