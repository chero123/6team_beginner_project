#성능 비어있거나 갯수가 적은 클래스들을 채워주고나서도 정답률이 떨어지는 클래스들 추가 50개씩 크롭
...
# ✅ key=dl_idx, value="... (cls N)" 포함
MAP_PATH = PROJECT_ROOT / "category_id_mapping.json"

# ✅ 출력 폴더(새로)
OUT_ROOT = PROJECT_ROOT / "cropped_add_targets50_raw_v1"
...
# ✅ 성능 낮은 타겟 클래스 (cls 기준)
# =========================================================
TARGET_CLASSES = {4, 8, 11, 17, 18, 19, 27, 30, 45, 47, 49, 61}

MAX_PER_CLASS = 50     # ✅ 클래스당 최대 몇 장 저장할지
PAD_RATIO = 0.20       # bbox 주변 여백 포함
MIN_CROP_WH = 32
...
def crop_with_padding(img, bbox_xywh: Tuple[float, float, float, float], pad_ratio: float):
    """
    bbox_xywh: COCO (x, y, w, h) pixel 좌표
    return: (cropped_img, new_bbox_xywh_in_cropped)
    """
    H, W = img.shape[:2]
    x, y, bw, bh = bbox_xywh
...
def main():
    ...
    # 1) dl_idx -> cls 만들기 (category_id_mapping.json 활용)
